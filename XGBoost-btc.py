...
future_df_scaled = scaler.transform(future_df[['day', 'month', 'year'] + [f'lag_{lag}' for lag in range(1, 8)]])

# Predict future prices
future_predictions = model.predict(future_df_scaled)

# Using the last known prices as a base for lag features in the forecast period
for i in range(len(future_df)):
    for lag in range(1, 8):
        if i < lag:
            future_df.loc[i, f'lag_{lag}'] = btc_data['Close'].iloc[-lag + i]
        else:
            future_df.loc[i, f'lag_{lag}'] = future_predictions[i - lag]
